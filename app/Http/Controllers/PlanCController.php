...
//                return redirect()->route('dashboard');
    ...
    {
        $this->setPageHeader('Plan-C Board');
    ...
    /**
     * Menampilkan halaman untuk join Plan-C
     *
    ...
    public function join(Request $request)
    {
        ...
        /*if (!$clsPlanC->isFlayed($currentPlanC)) {
            $pesan = $this->setPesanFlash('error', 'Anda sudah bergabung di Plan-C.<br/>Anda tidak dapat bergabung kembali sebelum status Anda di Plan-C sudah Fly');
            return redirect()->route('planc')->with($pesan);
        }*/
        $this->setPageHeader('Join Plan-C', 'Join to NuLife Plan-C program');
        ...
        if ($request->isMethod('post')) {
            ...
            if ($mySummaryPin->total_pin <= 0) {
                $pesan = $this->setPesanFlash('error', 'You have no PIN to join NuLife Plan-C program');
                return redirect()->route('planc')->with($pesan);
            }
            ...
            if (!$this->PlanC->canRegisterByCount($this->user)) {
                $pesan = $this->setPesanFlash('error', 'Your account in this plan-C has been maxed out.');
                return redirect()->back()->with($pesan);
            }
            $myPlanC = null;
            if ($this->PlanC->registerPlanC($this->user, $mySummaryPin->usable_id, $myPlanC)) {
                $myCodeC = (empty($myPlanC)) ? 'ERROR' : $myPlanC->plan_c_code;
                $pesan = $this->setPesanFlash('success', 'You have successfully join NuLife Plan-C program, Your Plan-C ID is <strong>' . $myCodeC . '</strong>');
                return redirect()->route('planc')->with($pesan);
            } else {
                $pesan = $this->setPesanFlash('error', 'Join NuLife Plan-C program process failed.');
    ...
    /**
     * Menampilkan halaman untuk order pin plan-C
     *
     * @param \Illuminate\Http\Request
     * @return \Illuminate\Http\Response
     */
    public function transferInstruction(Request $request) {
        ...
        $this->setPageHeader('Order Pin Plan-C', 'PIN Order for NuLife Plan-C program');
    ...
    /**
     * Menampilkan halaman status order pin Plan-C
     *
     * @return \Illuminate\Http\Response
     */
    public function statusOrder(Request $request)
    {
        $this->setPageHeader('NuLife Plan-C PIN Order Status');
    ...
    public function bonusList(Request $request) {
        ...
        //$this->setPageHeader('Bonus Plan-C', 'List of NuLife Plan-C Bonus You have earned.', false, true, 0, 0, $sisaBonus);
        $this->setPageHeader('Bonus Plan-C', 'List of NuLife Plan-C Bonus You have earned.');
    ...
    public function bonusSuccessList() {
        $bonus = $this->PlanC->getHistoryBonus($this->user);
        //$sisaBonus = $this->PlanC->getSummaryBonusFromList($bonus);
        //$this->setPageHeader('Withdrawed Bonus Plan-C', 'List of NuLife Plan-C Bonus You have earned and withdrawed.', false, true, 0, 0, $sisaBonus);
        $this->setPageHeader('Withdrawed Bonus Plan-C', 'List of NuLife Plan-C Bonus You have earned and withdrawed.');
    ...
    public function userBank(Request $request) {
        ...
        $this->setPageHeader('NuLife Plan-C Account Bank', 'This facility is provided to facilitate the process of withdrawal of the bonus plan-c');
    ...
    public function uploadBuktiTransfer(Request $request)
    {
        ...
        $pesan = $this->setPesanFlash('success', 'Upload proof of transfer, for your Plan-C PIN order, Success.');
        return redirect()->route('planc.status.order')->with($pesan);
    }

    public function historyPlan(Request $request) {
        $this->setPageHeader('Plan-C History', 'History of joined account to Plan-C');
