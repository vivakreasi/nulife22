...
class PinBController extends Controller
{
    /**
    ...
    public function myPin(Request $request)
    {
        ...
        $status = 0;
        return view('pinb.my', compact( 'jmlPin', 'status' ));
    }
...
//        dd($stockist_list);
    ...
    public function orderPin(Request $request)
    {
        ...
        $this->setPageHeader('Order/Transfer PIN', 'PIN Order/Transfer for NuLife Plan-B program');
...
//                    $min_order = \App\MinOrderSetting::where('name', 'Stockis')->value('amount');
        ...
        $pin_type_list = \App\PinBType::first();
        ...
        if ($request->isMethod('post')){
            ...
            $transaction = new \App\TransactionB;
            ...
            $trList     = new \App\TransactionBList;
            ...
            DB::commit();
            $pesan = $this->setPesanFlash('success', 'Your purchase order has been received and Recorded in our database.');
            return redirect()->route('pinb.invoice', ['transaction_code' => $trans_code])->with($pesan);
            
        }
        return view('pinb.order')
...
//                    $min_order = \App\MinOrderSetting::where('name', 'Stockis')->value('amount');
    ...
    public function invoicePin($transaction_code)
    {
        ...
        $transaction = \App\TransactionB::where('transaction_code', $transaction_code)
        ...
        $list = \App\TransactionBList::where('transaction_code', $transaction->transaction_code)->get();
        ...
        $confirm_order = \App\TransactionBConfirm::where('transaction_code', $transaction_code)->first();

        return view('pinb.invoice', compact( 'transaction', 'list', 'from', 'confirm_order' ));
    }

    public function listOrder()
    {
        $this->setPageHeader('List Order Pin');
        return view('pinb.list_order')
                ->with('status', 0);
    }
    ...
    {
        $data = \App\TransactionB::find($request->id);
        if($request->status == 2){
            DB::beginTransaction();
            $pin_type_list = \App\PinBType::first();
            ...
            if($data->transaction_type == 3){
                $transaction_list = \App\TransactionBList::where('transaction_code', $data->transaction_code)
                ...
                if($transaction_list->amount > 0){
                    ...
                    for ($x=0; $x < $transaction_list->amount; $x++) { 
                        ...
                        */
                        ...
                        $transaction_detail[$x] = new \App\TransactionBDetail;
                ...
                if($transaction_list->amount > 0){
                    $getPin = \App\PinBMember::where('user_id', $data->from)
                                                    ->where('is_used', 0)
                                                    ->where('pin_type_id', $pin_type_list->id)
                                                    ->get();
                    ...
                    for ($x=0; $x < $transaction_list->amount; $x++) { 
                        ...
                        $pin_member = \App\PinBMember::find($getPin[$x]->id);
        ...
        if ($request->isMethod('post')){
            ...
            try {
                ...
                */
                ...
                $transaction_list = new \App\TransactionBList;
                ...
                foreach ($freePin as $rowPin) {
                    ...
                    $pin_member = \App\PinBMember::where('id', '=', $rowPin->id)->update(['user_id' => $transaction->to, 'transaction_code' => $trans_code]);
    ...
    {
        $data = new \App\TransactionBConfirm;
        ...
        \App\TransactionB::where('transaction_code', $request->transaction_code)->update(['status' => 1]);
