...
class StrukturJaringan extends Model 
{
    protected $table = 'tb_structure';
    ...
    public function getStructureByUserId($userId) {
        return $this->leftJoin('users', 'tb_structure.user_id', '=', 'users.id')
                    ->leftJoin('users_type', 'users_type.id', '=', 'users.id_type')
                    ->selectRaw("tb_structure.*, users.name, users.id_type, users.plan_status, users_type.code, users_type.desc, users_type.short_desc")
                    ->where('tb_structure.userid', '=', $userId)->first();
    }
            ...
            if (!array_key_exists($struktur->id, $cStruktur)) {
                ...
                if (!$kosong) {
                    if ($struktur->level > 1) {
                        if (!empty($row->name)) {
                            if ($placement) {
                                $goUrl = route('plan.placement', $dataId);
                            } else {
                                $goUrl = route('plan.network.binary', $dataId);
                            }
                        ...
                        if (strtoupper($owner->userid) != $dataId) {
                            ...
                            for ($s = 1; $s <= $maxLevel - 1; $s++) {
                                if ($placement) {
                                    $goUrl = route('plan.placement', [$dataId, $s]);
                                } else {
                                    $goUrl = route('plan.network.binary', [$dataId, $s]);
                                }
            ...
            if (!empty($dataTop)) {
                ...
                foreach ($strukturs as $key => $value) {
                    ...
                    if ($level > 1) {
                        $kaki = $value->kaki;
                        if (!empty($parentData)) {
                            $childData  = $this->leftJoin('users', 'tb_structure.user_id', '=', 'users.id')
                                            ->leftJoin('users_type', 'users_type.id', '=', 'users.id_type')
                                            ->selectRaw("tb_structure.*, users.name, users.id_type, users.plan_status, users_type.code, users_type.desc, users_type.short_desc")
                                            ->where('tb_structure.upline_id', '=', $parentData->user_id)
                                            ->where('tb_structure.foot', '=', $kaki)
                                            ->first();
                        } else {
                            $childData = null;
                        }
    ...
    public function getActualStructure($userOwner, $userTarget) {
        ...
        $data = $this->leftJoin('users', 'tb_structure.userid', '=', 'users.userid')
            ->selectRaw("tb_structure.id, tb_structure.kode, tb_structure.userid, tb_structure.level - " . $start->level . " AS level, tb_structure.foot, 
                        users.name, users.created_at, users.plan_status, 
                        '0' as parent_kode, 0 as kiri, 0 as kanan")
            ->where('tb_structure.kode', 'LIKE', $owner->kode . '%')
            ->where('tb_structure.kode', 'LIKE', $start->kode . '%')
            ->whereBetween('tb_structure.level', [$start->level, $start->level + $this->showActualMaxLevel - 1])
            ->orderBy('tb_structure.kode')
            ->get();
        if (!empty($data)) {
            foreach ($data as $row) {
                /*$rowKode    = $row->kode . '-%';
    ...
    public function getRequirePlacement($user) {
        ...
        return User::where('users.id_referal', '=', $user->id)
                        ->leftJoin('tb_structure', 'tb_structure.user_id', '=', 'users.id')
    ...
    public function isUserExist($userId) {
        ...
        $data   = User::where('users.userid', '=', $userId)
                        ->join('tb_structure', 'tb_structure.user_id', '=', 'users.id')
                        ->selectRaw('users.id, users.userid, users.name, users.email, users.active_at, users.created_at')
                        ->where('users.is_active', '=', 1)
                        ->first();
        return !empty($data);
    ...
    public function getListDirectSponsored($user) {
        ...
        $data   = User::where('users.id_referal', '=', $user->id)
                        ...
                        ->selectRaw('users.userid, users.name, 
                            (CASE WHEN tb_structure.id IS NULL THEN 0 ELSE 1 END) AS status,
                            tb_structure.kode, COALESCE(tb_structure.level, 0) AS level, users.id, users.is_active')
                        ->where('users.is_active', '=', 1)
                        ->get();
        return $data;
    }
    ...
    public function summaryStructure($user) {
        ...
        $mine   = $this->where('tb_structure.user_id', '=', $user->id)->first();
        if (empty($mine)) return $result;
        $PerLevel = $this->where('tb_structure.kode', 'LIKE', $mine->kode . '-%')
                        ->join(DB::raw("(" . generateSqlLevel($mine->level, 10) . ") AS levelan"), 'levelan.level', '=', 'tb_structure.level')
                        ->selectRaw("tb_structure.level, levelan.show_level, COUNT(tb_structure.id) AS jml_per_level")
                        ->groupBy('tb_structure.level')
        ...
        $result->sponsored->Activated       = $sponsored1->whereNotNull('tb_structure.id')->count('users.id');
        $result->sponsored->UnActivated     = $sponsored2->whereNull('tb_structure.id')->count('users.id');
        /*
        $result->globalSummary->Yesterday   = $this->where('tb_structure.kode', 'LIKE', $mine->kode . '-%')
                                                ->where(DB::raw("DATE_FORMAT(created_at, '%Y-%m-%d')"), '=', DB::raw("DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 DAY), '%Y-%m-%d')"))
                                                ->count('tb_structure.id');
        $result->globalSummary->Today       = $this->where('tb_structure.kode', 'LIKE', $mine->kode . '-%')
                                                ->where(DB::raw("DATE_FORMAT(created_at, '%Y-%m-%d')"), '=', DB::raw("DATE_FORMAT(NOW(), '%Y-%m-%d')"))
                                                ->count('tb_structure.id');
        $result->globalSummary->ThisWeek    = $this->where('tb_structure.kode', 'LIKE', $mine->kode . '-%')
                                                ->where("created_at", '>', DB::raw("DATE_SUB(NOW(), INTERVAL 1 WEEK)"))
                                                ->count('tb_structure.id');
        $result->globalSummary->ThisMonth   = $this->where('tb_structure.kode', 'LIKE', $mine->kode . '-%')
    ...
    private function addStructure($parentStructure, $kode, $upline, $user, $foot, &$newData) {
        $values = array(
            'kode'      => $kode, 
            'user_id'   => $user->id, 
            'userid'    => $user->userid, 
    ...
    private function getPairQuery($parent, $level, $fromChildFoot) {
        ...
        return $this->leftJoin('tb_bonus_pairing', function($join) use($parent, $fromLeft) {
                        if ($fromLeft) {
                            $join->on('tb_bonus_pairing.left_id', '=', 'tb_structure.id');
                        } else {
                            $join->on('tb_bonus_pairing.right_id', '=', 'tb_structure.id');
                        }
                        $join->on('tb_bonus_pairing.pair_level', '=', 'tb_structure.level');
                        $join->on('tb_bonus_pairing.user_id', '=', DB::raw($parent->user_id));
                    })
                    ->selectRaw("tb_structure.*")
                    ->whereRaw("tb_structure.kode LIKE '" . $kode . "'")
                    //->whereRaw("tb_structure.level = " . $level)
                    //->where(DB::raw("DATE_FORMAT(tb_structure.created_at, '%Y-%m-%d')"), '=', DB::raw("DATE_FORMAT(NOW(), '%Y-%m-%d')"))
    ...
    private function getPairs($settings, $clsPairing, $kode, $level, &$needRankUp) {
        ...
        foreach ($parentCodes as $value) {
            ...
            //$query      = $this->getPairQueryToday($parent, $level, $value->pair_foot)->orderBy('tb_structure.kode')->first();
            /*$query      = $this->getPairQuery($parent, $level, $value->pair_foot)
                                ->orderBy('tb_structure.level')
                                ->orderBy('tb_structure.kode')
                                ->first();
    ...
    public function placeMember($owner, $user, $parentCode, $foot, $pin) {
        if (empty($owner) || empty($user) || $parentCode == '' || !in_array($foot, [1, 2]) || empty($pin)) return false;
        ...
        if ($pin->is_used != 0) return false;

        $sponsor = ($owner->id == $user->id_referal) ? $owner : User::where('id', '=', $user->id_referal)->first();
        if (empty($sponsor)) return false;

        ini_set('memory_limit', '1024M');
        set_time_limit(0);

        $clsPairing = new \App\Bonus_pairing;
        $clsSponsor = new \App\Bonus_sponsor;
        $settings   = $this->getBonusSetting();

        $level  = $parent->level + 1;
        $kode   = AppTools::getStructureCodeByFoot($parent->kode, $level, $foot, '.', '-');
        $needRankUp     = [];
        //$pairDatas      = $this->getPairs($settings, $clsPairing, $kode, $level, $needRankUp, $owner);
        $pairDatas      = $this->getPairs($settings, $clsPairing, $kode, $level, $needRankUp);
        
        $bonusSponsor   = $clsSponsor->getBonus($sponsor, $settings);

        DB::beginTransaction();
        $newData = null;
        if ($this->addStructure($parent, $kode, $upline, $user, $foot, $newData)) {
            if ($bonusSponsor->get) {
                if (!$clsSponsor->createBonus($sponsor, $user, $bonusSponsor->bonusSplited)) {
                    DB::rollback();
                    return false;
                }
            }
            if (!empty($pairDatas)) {
                if (!$clsPairing->createBonus($newData, $pairDatas)) {
                    DB::rollback();
                    return false;
                }
            }
            if (!empty($needRankUp)) {
                $suksesRankUp = true;
                foreach ($needRankUp as $userRankUp) {
                    if (!$userRankUp->rankUpMe()) {
                        $suksesRankUp = false;
                        break;
                    }
                }
